class Calculadora{
    ...
    // **********************************
    // * NUEVA FUNCIONALIDAD: CAMBIAR SIGNO *
    // **********************************
    cambiarSigno() {
        // 1. Evita cambiar el signo si el valor actual es "0"
        if (this.ValorActual === '0') return;

        // 2. Comprueba si el número ya es negativo
        if (this.ValorActual.startsWith('-')) {
            // Si es negativo, elimina el signo (-) para hacerlo positivo
            this.ValorActual = this.ValorActual.substring(1);
        } else {
            // Si es positivo, añade el signo (-) al inicio para hacerlo negativo
            this.ValorActual = '-' + this.ValorActual;
        }
        
        // 3. Actualiza la pantalla para reflejar el cambio
        this.actPantalla();
    }
